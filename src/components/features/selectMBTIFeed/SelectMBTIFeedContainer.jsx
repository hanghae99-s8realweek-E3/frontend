...
function SelectMBTIFeedContainer () {
  ...
  const mbtiList = ["ISFJ", "ISTJ", "INFJ", "INTJ", "ISTP", "ISFP", "INFP", "INTP", "ESTP", "ESFP", "ENFP", "ENTP", "ESTJ", "ESFJ", "ENFJ", "ENTJ"]
  const [selectMBTI, setSelectMBTI] = useState(MyData === undefined || MyData.mbti === null ? "" : MyData.mbti);
  ...
  function buttonColorSet (elem, index) {
    if (mbtiList[index] === selectMBTI) {
      if (mbtiState.mbti === selectMBTI) 
        return "gray"
      else
        return "#E8644C"}
    else if (Object.keys(mbtiState).length === 0)
      return "#ffffff"
    else if (mbtiList[index] === mbtiState.mbti) 
      return "gray"
    else if (mbtiState[elem] === 5)
      return "#507DF1"
    else if (mbtiState[elem] === 1)
      return "#FFD600"
    else
      return "#ffffff"
  }
        ...
        <StGrid>
          {mbtiList.map((elem, index) => {
            return <StButton
                      ...
                      border={buttonColorSet(elem, index) === "#ffffff" ? "1px solid #979797" : "none"} 
                      color={buttonColorSet(elem, index) === "#ffffff" ? "#979797" : "#ffffff"}
                      onClick={buttonColorSet (elem, index) === "gray" ? null : selectMBTIFeed}
                      cursor={buttonColorSet (elem, index) === "gray" ? "arrow" : "pointer"}
                      value={mbtiList[index]}
                      >{mbtiList[index]}</StButton>
          })}
        </StGrid>
      </div>
